...
class Fighter:
    def __init__(self, player,x,y, char_name="YURI", is_cpu =False):
        self.player = player
        ...
        if not self.attacking and not self.attackingc:
            ...
            if self.player == 2:
                if not self.isCpu:
                    ...
                    distancia = abs(self.rect.centerx - target.rect.centerx)
                    self.cpuTimer += 1

                    ReactionFrames = 30

                    if self.cpuTimer >= ReactionFrames:
                        self.cpuTimer = 0

                        if (target.attacking or target.attackingc) and random.random() < 0.6:
                            self.cpu_state = "block"
                        elif distancia < 90:
                            self.cpu_state = "attack"
                        else:
                            self.cpu_state = "approach"

                    if self.cpu_state == "block":
                        self.isBlocking = True
                        self.isCrouching = False
                        if target.rect.centerx > self.rect.centerx:
                            dx = -Speed * 0.5
                        ...
                        if random.random() < 0.03 and not self.isJumping:
                            self.vel_y = self.jump_force
                            self.isJumping = True
                    ...
                    if not self.attacking and not self.attackingc and self.cpu_state != "attack":
                        self.isCrouching = False
